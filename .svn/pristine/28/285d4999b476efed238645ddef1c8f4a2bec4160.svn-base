...
/*****************************************************************************
*
*   @ SYSTEM NAME       : 신용카드 Index 페이지(모바일)
*   @ PROGRAM NAME      : index_card_m.jsp
*   @ MAKER             : InnoPay PG
...
<%
System.out.println("Start ipay index_card_m.jsp*****************["+System.currentTimeMillis()+"]");
...
function goNext() {
    ...
    var mispForm = document.m_ispform;
...
//    disableItems(true);
    ...
    if(contains(foreignCardArray, bankCd)){
        // 해외카드는 현재 구현 안됨
        formNm.action = "inputForeign_card_m.jsp";
        formNm.submit();
    }else{
        // 포인트결제 적용하는 경우 아래 구현 
        /*
        ...
        */
        ...
        if(contains(ispArray,bankCd)){
        	...
        	document.m_ispform.PgId.value = kvp_pgid;
        	formNm.KVP_PGID.value=document.m_ispform.PgId.value;
            if(bankCd == '<%=CommonConstants.CARD_CD_KB%>'){
            	mispForm.CardCode.value = "0204";
                formNm.bankCd_ispm.value = "0204";
                formNm.co_no.value='<%=co_no%>';
                formNm.co_nm.value='<%=co_nm%>';
                formNm.fn_no.value=fnNo;
                formNm.KVP_QUOTA_INF.value='<%=kvpQuota%>';
                document.m_ispform.Noint_Inf.value = cardQuota;
...
//                install_notice_on();
            	...
            	if(bankCd == '<%=CommonConstants.CARD_CD_BC%>'){
            		mispForm.CardCode.value = "0100";
                    formNm.bankCd_ispm.value = "0100";
                }else{
                	mispForm.CardCode.value = "ALL";
                }
            	document.m_ispform.Noint_Inf.value = cardQuota;
                initISP_M();
                return false;
        ...
        // 안심클릭
            ...
            if (bankCd == '<%=CommonConstants.CARD_CD_LOTTE%>'){  // 로코모
            	document.tranMgr.fn_no.value = fnNo;
            	
            	var chain_code_val = '0';
                if($("#app_chain_code").is(":checked")){
                    chain_code_val = '3';
                }
                document.tranMgr.sp_chain_code.value = chain_code_val; // 0:일반결제, 1:간편(삼성,신한,현대), 3:앱카드                
                doXansim();
                return false;
...
<input type="hidden" name="MerchantNo_ispm" value="">
<input type="hidden" name="bankCd_ispm" value="">
<input type="hidden" name="issuerCode" value="">
<input type="hidden" name="Noint_Inf" value="" >
...
<input type="hidden" name="fn_no" value="">
                    	...
                    	<div class="con_scroll">
	                        ...
	                        <section class="payment_input">
	                            ...
	                            <div>
	                                ...
	                                <li>
	                                    <div id="appCardDiv" class="checks" style="margin-bottom:10px;display:none;">
	                                ...
	                                <div class="input_section">
	                                    ...
	                                    <div class="input_type1">
	                                        <input type="email" id="BuyerEmail" name="BuyerEmail" pattern="[A-Za-z0-9]*" class="e_mail" value="<%=BuyerEmail%>" placeholder="이메일을 입력하세요(선택사항)" />
	                                    </div>
	                                </div>
	                            </div>
	                        </section>
                        ...
                        <div class="popup_cont">
                            <div class="popup_scroll">
                                <div class="popup_scroll_in">
                                    <iframe src="https://pg.innopay.co.kr/ipay/card/event.jsp?pg_cd=<%=pgCd %>&join_type=1" width="440px" height="1550px" frameborder="0" style="opacity: 1; visibility: visible;"><br /></iframe>                           
                                </div>
                            </div>
                        </div>  
...
※ 개인정보의 처리와 관련된 사항은 회사의 개인정보처리방침에 따릅니다.
                ...
                <section class="float_wrap card_frame">
                    <div class="pop_dim"></div>
                    <div class="popup card_pay">
                        <a href="#" class="btn_close pop_close"></a>
                        <iframe id="MpiFrame" name="MpiFrame" src="" frameborder="0" scrolling="no" style="overflow: hidden;"></iframe>
                    </div>
                </section>
                -->
...
<form name="XansimForm" id="XansimForm" method="POST" target="X_ANSIM_FRAME">
...
<iframe id="X_ANSIM_FRAME"  name="X_ANSIM_FRAME" style="display:none" src="./xansim/iframe.jsp" ></iframe>

<!-- XMPI Form & iframe 영역 종료 -->

<!-- 2016.02.02 by.sbs  -->
<!-- SMPI Form & iframe 영역시작 -->
<form name="spsForm" id="spsForm" method="POST" target="SPSFRAME">
<input type="hidden" name=order_amount 		value="">
<input type="hidden" name=order_currency 	value="">
<input type="hidden" name=order_cardname 	value="">
<input type="hidden" name=order_no 			value="">
...
<input type="hidden" name="mId" 			value="M201207130947M">
<input type="hidden" name=order_business 	value="">
<input type="hidden" name=order_mname 		value="">
...
<div id="SPSDIV" style="display:none;">
<iframe id="SPSFRAME" name="SPSFRAME" border="0" style="width:100%;height:100%;" scrolling="no"></iframe>
</div>
<!-- SMPI Form & iframe 영역종료 -->

<!-- VP ISP 관련 Form 시작 -->
<!-- Mobile isp test -->
<form name="m_ispform" method="post" action="">
<input type="hidden" name="LoginGubun" 		value="">    
<input type="hidden" name="PgId" 			value="10903">     <!-- PgId 체크해야함 -->
<input type="hidden" name="Tcode" 			value="">         <!-- 휴대폰 관련 테스트 확인후  -->
<input type="hidden" name="ReturnUrl" 		value="">
<input type="hidden" name="WAPUrl" 			value="">
<input type="hidden" name="Currency" 		value="WON">
<input type="hidden" name="CardCode" 		value="">
<input type="hidden" name="IpAddr" 			value="<%=UserIP%>">
...
<input type="hidden" name="NoInt" 			value="<%=CardInterest%>" >
<input type="hidden" name="Noint_Inf" 		value="" >
<input type="hidden" name="HpNum" 			value="<%=BuyerTel%>" >       <!-- 휴대폰 관련 테스트 확인후  -->
</form>
<!-- VP ISP 관련 Form 종료 -->

<!-- ILKMPI Form & iframe 영역 시작-->
<IFRAME id=ILKFRAME name=ILKFRAME style="display:none" src="./ilk/dummy.jsp"></IFRAME>
