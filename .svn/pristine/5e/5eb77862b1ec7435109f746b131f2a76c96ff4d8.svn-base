...
function setPayActionUrl(val) {
    ...
    //payActionUrl='http://172.16.10.10:8080';
    //payActionUrl='http://180.67.115.172:8182';  // 테스트URL 셋팅
};
...
function checkData(frm, data){
	if('CARD'!=data.PayMethod&&'BANK'!=data.PayMethod&&'VBANK'!=data.PayMethod&&'CARS'!=data.PayMethod&&'CSMS'!=data.PayMethod&&'DSMS'!=data.PayMethod&&'CKEYIN'!=data.PayMethod&&'EBANK'!=data.PayMethod){
	...
	if('CARD'==data.PayMethod){
		...
		iw=680;
		ih=740;
		jQuery('[name=PayMethod]',jQuery(frm)).val('CSMS');
