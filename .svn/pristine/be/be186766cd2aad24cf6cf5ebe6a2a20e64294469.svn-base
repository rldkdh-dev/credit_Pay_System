...
/******************************************************************************
*
*	@ SYSTEM NAME		: 전문전송 (간편결제 BLD)
*	@ PROGRAM NAME		: payTrans_EPay.jsp
*	@ MAKER				: 
*	@ MAKE DATE			: 2020.04.08
*	@ PROGRAM CONTENTS	: 전문전송 (간편결제 BLD)
*
************************** 변 경 이 력 *****************************************
* 번호	작업자		작업일				변경내용
*******************************************************************************/
...
<meta charset="utf-8">
<%
System.out.println("**** Start /card/payTrans_EPay.jsp ****");
...
if (eNames.hasMoreElements()) {
	...
	System.out.println("EPay payTrans_EPay.jsp  [device check]  " + device);
	Box box = new Box();
...
**/
	...
	if(EPayCl.equals("04"))
	req.put("PayAmt"		,SSGPAY_CARD_TRADE_AMT);
	else
	req.put("PayAmt"		,Amt);
	
	req.put("MoneyAmt"		,SSGPAY_MGIFT_TRADE_AMT);
	...
	req.put("CardPoint"		,cardType);
	req.put("EPayCl"		,EPayCl);	
	req.put("CardType"		,cardType);
	req.put("AuthFlg"		,"02");
	req.put("CardBankCode"	,svcPrdtCd);
...
%>
    ...
    <%if(resResultCode.equals("3001")){%>
    	formNm.action = "./payResult_EPay.jsp";
    <%}else{%>
    	formNm.action = "./payResultFail_EPay.jsp";
    <%}%>
    	formNm.submit();
    	return true;
    }
