...
/******************************************************************************
*
*	@ SYSTEM NAME		: 전문전송 (신용카드 수기 결제)
*	@ PROGRAM NAME		: payTransKeyin.jsp
*	@ MAKER				: 
*	@ MAKE DATE			: 2019.01.22
*	@ PROGRAM CONTENTS	: 전문전송 (신용카드 수기 결제)
*
************************** 변 경 이 력 *****************************************
* 번호	작업자		작업일				변경내용
*******************************************************************************/
...
<%
System.out.println("**** Start /card/payTransKeyin.jsp ****");
...
if (eNames.hasMoreElements()) {
	...
	System.out.println("**** CARD payTransKeyin.jsp  [device check]  " + device);
	Box box = new Box();
...
**/
    ...
    if(!"pgbcdplatm".equals(MID)) {
	    if(!EncryptData.equals(strEncData)){
	    	System.out.println("**********payTransKeyin [거래검증 데이터가 일치하지 않습니다]**********");
    ...
    req = context.newGiftBox("NPG01WCD01"); // 카드 수기 결제승인 요청 전문
	...
	if(BuyerAuthNum.length() == 10) { // 법인
		cardType = "02";
	} else if(BuyerAuthNum.length() == 6) { // 개인
		cardType = "01";
	} else {
		cardType = "01";
	}
