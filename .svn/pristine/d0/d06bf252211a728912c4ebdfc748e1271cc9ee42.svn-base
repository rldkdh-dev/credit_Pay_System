...
<script language="javascript">
	...
	function _submit() {
		...
		//parent.document.getElementById("DIV_KAKAOFRAME").innerHTML = "<IFRAME id='KAKAO_IFRM' name='KAKAO_IFRM' width=100% height='500px' scrolling=no frameborder='0' ></IFRAME>";
		document.getElementById("KAKAO_IFRM").style.visibility="visible";
		vfrm.returnUrl.value = getLocalUrl("kakao_return.jsp");
		if (isMobile()){
			vfrm.processtype.value="2";   // PC:1, mobile : 2 -->
		}
...
function kakao_cancel(){ //카카오 취소버튼 눌렀을 경우....
		var frm = document.KSPayAuthForm;
		frm.action="../EPay_mobile.jsp"
		frm.submit();	
	}
</script>
</head>
<body onload="_submit();" >
<form name="KSPayAuthForm" method="post">  <!-- 취소, 결제 인증 파라미터 Form -->
<!--기본-------------------------------------------------------------->
...
<input type="hidden" name="device"             value="<%=device%>">	<!-- 모바일 ,PC 구분 -->
<input type="hidden" name="OfferingPeriod"     value="<%=OfferingPeriod%>"/>
</form>
	
<form name="kfrm" target="_blank" method="post">  <!-- 카카오페이지 결제 페이지 호출 Form -->
	<INPUT type="hidden" name="returnUrl"      value="" >                 <!-- returnUrl -->
	...
	<INPUT type="hidden" name="processtype"    value="2" >                 <!-- PC:1, mobile : 2 -->
...
<div class="class="iPhone_scroll"">
<IFRAME id='KAKAO_IFRM' name='KAKAO_IFRM' width=100% height='100%' frameborder='0' ></IFRAME>
</div>
</body>
</html>