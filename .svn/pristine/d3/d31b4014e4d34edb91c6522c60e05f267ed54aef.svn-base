...
<%
	...
	List<DataModel> aList = insurance.getInsuranceReqList(param);
	
	if(aList.size() > 0){
		DataModel map = (DataModel)aList.get(0);
		
		if(map.getStrNull("recv_yn").equals("N")){
			
			insurance.updateInsuranceReqRecv(param);
		...
		if(aList2.size() > 0){
			...
			map.put("co_nm", map2.getStrNull("co_nm"));
		...
		DataModel useMap = insurance.getInsuranceServiceUseInfo(map);
	  	...
	  	try{
	  		...
	  		if(!map.getStrNull("recv_yn").equals("Y")){
				...
				mailMessage.setContent(insurance.getTemplate03(map)+"");
