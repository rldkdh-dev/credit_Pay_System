...
<%!
	...
	public Map CallCredit(Map REQ_DATA, boolean Debug, String CPID, String CRYPTOKEY) {

		String REQ_STR = toEncrypt(data2str(REQ_DATA), CRYPTOKEY);
		...
		if (RES_STR != null && !"".equals(RES_STR)) { //암호화된 정상응답값
			RES_STR = toDecrypt((String) resMap.get("DATA"), CRYPTOKEY);
