...
/******************************************************************************
*
*   @ SYSTEM NAME       : 실시간계좌이체 메인페이지
*   @ PROGRAM NAME      : mainBank_m.jsp
*   @ MAKER             : 
*   @ MAKE DATE         : 2018.03.06
*   @ PROGRAM CONTENTS  : 실시간계좌이체 메인페이지
*
************************** 변 경 이 력 *****************************************
* 번호    작업자     작업일             변경내용
*******************************************************************************/
...
<%
    ...
    String TID = KeyUtils.genTID(MID, CommonConstants.SVC_CD_BANK, "01");
    ...
    if(Integer.parseInt(Amt.equals("")?"0":Amt) <=500){
        throw new Exception("W021"); // 500원 이하 결제불가
    }
    ...
    // 금결원 CPID 정보 가져오기 
    Box box = null;    
    Box resCPIDInfo = null;
    box = new Box();
    box.put("mid", MID);    
    box.put("svc_cd", svcCd);
    box.put("svc_prdt_cd", svcPrdtCd);
    box.put("today", TimeUtils.getyyyyMMdd());
        
    resCPIDInfo = CommonBiz.getCPIDInfo(box);
    String cpid = null;
    if(resCPIDInfo!=null){
        cpid = resCPIDInfo.getString("fn_no");
    }
    if(resCPIDInfo == null || cpid == null || "".equals(cpid)) {
        throw new Exception("W020");
    }

    ///////////////////////////////////
    
    // 현금영수증 정보 가져오기
    String cash_rcpt_auto_cl = resMemberInfo.getString("cash_rcpt_auto_cl");
    
    //NOTICE: 이외값은 모두 고정값이라고 보면됨.
    /////////////////////////////////////////
    // 금결원 계좌이체 관련 정보 설정 승인번호 
    //String hd_approve_no = "20000523"; //1일 1회 결제 한도 200만원
    String hd_approve_no = cpid; // 금결원 CPID
    String hd_firm_name = resMemberInfo.getString("co_nm"); //회원사명
    String co_no = resMemberInfo.getString("co_no"); //사업자번호
    String co_nm = resMemberInfo.getString("co_nm");   // 상호(가맹점)
    String tel_no = resMemberInfo.getString("tel_no"); //대표전화번호
    String tx_amount = Amt; // 금액
    /* 예비 필드 80바이트 
    PG사의 경우: tx_user_define value="업체명(통장인자용, 5자리)^사업자번호^업체명(전체)^대표전화번호"
     예: value="테스트기관^1234567890^Bankpay계좌이체테스트기관^02-531-1825" 
    */  
    // TODO : 계좌이체 사용
    String tx_user_define =  hd_firm_name + "^" + co_no + "^" + hd_firm_name + "^" + tel_no;
    String tx_receipt_acnt = "46440104105665"; // For REAL
    String tx_email_addr = BuyerEmail; // 구매자 email 없으면 공백, 금결원 ActiveX에서 입력
    
    String tx_user_key = TID;
    //System.out.println("tx_user_key: " + tx_user_key);
    String callbackfunc = payActionUrl + "/ipay/bank/returnBankPay.jsp";
...
function goNext() {
    ...
    // 휴대폰 입력 확인        
    var BuyerTel = '';
    var BuyerTel1 = formNm.BuyerTel1.value;
    var BuyerTel2 = formNm.BuyerTel2.value;
    var BuyerTel3 = formNm.BuyerTel3.value;
    BuyerTel = BuyerTel1 + BuyerTel2 + BuyerTel3;
    
    if(BuyerTel.length < 10) {
        alert("휴대폰번호를 입력하세요.");
        formNm.BuyerTel1.focus();
        return false;
    }else{
        formNm.BuyerTel.value = BuyerTel;
    }
    <%-- 현금영수증 발급 체크 --%>
    if($('input:radio[name="cashReceiptType"]:checked').val() == "") {
		...
		if($('input:radio[name="cashReceiptType"]:checked').val() == "0") {
        ...
        else if($('input:radio[name="cashReceiptType"]:checked').val() == "3"){
            formNm.tx_bill_yn.value='N';
        }
...
function  callActivity(){
	   ...
	   try{
		   ...
		   if((opener!=undefined && opener!=null) || 'X'=='<%=FORWARD%>'){
...
<body>
<form name="tranMgr" method="post" action="./sendBankTransinfo.jsp" >
<%-- 상점에서 입력하는 파라미터 --%>
...
<input type="hidden" name="hd_pre_msg_type" value="EFT">
...
<!-- 계좌이체는 CERT로 고정 모바일TEST 는 NOCERT -->
<input type="hidden" name="hd_ep_type"      value="SECUCERT">
<!-- <input type="hidden" name="hd_ep_type"      value="NOCERT"> -->
<!-- 공백으로 고정 -->
...
<!-- 결제 요청 전문의 대한 일련번호 7자리 : 중복거래를 방지 -->
...
<!-- 현금영수증 발급여부 -->
<input type="hidden" name="tx_bill_yn"       value="N">
<!-- 부가세 대상 여부 -->
<input type="hidden" name="tx_vat_yn"       value="Y">
<!-- 예비 필드 80바이트 : 이용기관에서 자유롭게 사용할 수 있는 필드임 : ID, 주문상품명 등 -->
<!-- 일반PG사의 경우 다음의 형식에 맞게 입력해야 함 : 
     value="업체명(통장인자용, 5자리)^사업자번호^업체명(전체)^대표전화번호"
     예: value="테스트기관^1234567890^Bankpay계좌이체테스트기관^02-531-1825" -->
...
<input type="hidden" name="hd_input_option" value="22">
<!-- 출력옵션설정: 111~444 중 선택 : 전자지갑 결과창에 결제결과가 표시되는 방법에 대한 옵션임 -->
<input type="hidden" name="hd_ep_option"    value="222">
<!-- 전자지갑 타임아웃 설정: 거래 없이 일정시간 지나면 종료되도록 하는 옵션임 -->
<input type="hidden" name="hd_timeout_yn"   value="Y">
...
<input type="hidden" name="callbackparam1" value=""/>
                        ...
                        <section class="payment_input">
                            ...
                            <div>
                                <div class="input_section">
                                    <label for="phone_num" class="input_title">휴대폰번호</label>
                                    <div class="phone_num input_type_split">
                                        <input name="BuyerTel1" type="number" id="BuyerTel1" value="" placeholder="010" pattern="[0-9]*" inputmode="numeric" maxLength="3" oninput="maxLengthCheck(this)" style="width: 34px;" />
                                        -
                                        <input type="number" name="BuyerTel2" id="BuyerTel2" value="" placeholder="1234" pattern="[0-9]*" inputmode="numeric"  maxLength="4" oninput="maxLengthCheck(this)"/>
                                        -
                                        <input type="number" name="BuyerTel3" id="BuyerTel3" value="" placeholder="1234"  pattern="[0-9]*" inputmode="numeric" maxLength="4" oninput="maxLengthCheck(this)"/>
                                    </div>
                                </div>
                                <!--  <div class="input_section">
                                ...
                                <div class="input_section">
									<div class="input_title">현금영수증</div>
									<div class="input_type1 radio">
										<div class="radio_wrap">
											<input type="radio" name="cashReceiptType" value="0" id="cashReceiptType3" onclick="javascript:OnCashReceiptType($(this).val());" checked>
                               ...
                               <!-- <div id="rcptType" class="input_section s3_2 s3">
                                ...
                                <div class="input_section">
                                    ...
                                    <div class="input_type1">
                                        <input type="email" name="BuyerEmail" id="BuyerEmail" value="<%=BuyerEmail %>" class="e_mail" pattern="[A-Za-z0-9]*" placeholder="이메일을 입력하세요"/>
                                    </div>
                                </div>
                            </div>
                        </section>
