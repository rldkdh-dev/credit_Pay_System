...
<%@ include file="./common_epay.jsp" %>
	<%
		System.out.println("**** Start ipay EPay.jsp ["+System.currentTimeMillis()+"] ****");
...
var lpay = null;
	...
	function goCancel() {
		...
		*/
		...
		if($("#simple_pay1_area").hasClass("on") === true){
		 ...
		 try{
			 install_notice_on();
			 KSPayAuthForm.storeid.value=MainForm.storeid.value
		     f = document.MainForm;
			
			 document.getElementById("SFRAME").style.visibility="visible";
			 f.target="SFRAME";
		     f.action = "kakao/kakao_pc.jsp";
			 f.submit();
		 }catch(e){
			 ...
			 //f = document.ka;
		    //f.action = "kakao/payConfirm_card_m.jsp";
			//f.submit();
				...
				try{
		            ...
		            document.getElementById("LPAYFRAME").style.visibility="visible";
					var call_url = "https://kspay.ksnet.to/store/PAY_PROXY/lpay/lpayp.jsp";  	 
					
					lpay.target = 'LPAYFRAME';
			 ...
			 try{
				    ...
				    EPay_btn_on();
               		document.getElementById("PAYCOFRAME").style.visibility="visible";
					var call_url = "https://kspay.ksnet.to/store/PAY_PROXY/payco/payco_p.jsp";  	 
					parent.$('#divpop').css("width","800px");	
					payco_frm.target = "PAYCOFRAME";
				...
				install_notice_on();
				var ssgfrm = document.SSGPayAuthForm;
				var frm    = document.KSPayAuthForm;
				ssgfrm.sndCertitype.value = "2"; //신용 타입 결제 지정
				ssgfrm.sndReply.value     = getLocalUrl('ssg/ssg_return.jsp');		
				try{					
					/*
					op = window.open('about:blank','SsgFrmUp',
					...
					*/					
					document.getElementById("SSGFRAME").style.visibility="visible";
					ssgfrm.target = 'SSGFRAME';					
					ssgfrm.action ='./ssg/ssg_pc.jsp'; 
					ssgfrm.submit(); 
				}catch(e){
					alert("결제창 호출에 실패하였습니다!");
				...
				var ssgfrm = document.SSGPayAuthForm;
				var frm    = document.KSPayAuthForm;
				ssgfrm.sndCertitype.value = "1"; //머니 타입 결제 지정
				ssgfrm.sndReply.value     = getLocalUrl('ssg/ssg_return.jsp');
				document.getElementById("SSGFRAME").style.visibility="visible";
				ssgfrm.target = 'SSGFRAME';					
				ssgfrm.action ='./ssg/ssg_pc.jsp'; 
				...
				var frm    = document.KSPayAuthForm;
				naverfrm.sndCertitype.value = "2"; //신용 타입 결제 지정
				naverfrm.sndReply.value     = getLocalUrl('naver/naver_return.jsp');	
				try{					
					/*
					op = window.open('about:blank','SsgFrmUp',
					        'height='+height_+',width='+width_+',status=yes,scrollbars=no,resizable=no,left='+left_+',top='+top_+'');					
					...
					*/					
					naverfrm.target = 'NAVERFRAME';					
					naverfrm.action ='./naver/naver_pc.jsp'; 
					naverfrm.submit(); 
				}catch(e){
					alert("결제창 호출에 실패하였습니다!");				
				    throw e;
		...
		function recvResult(proceed){
			...
			if(proceed == "TRUE"||proceed == "true"||proceed == true){
					...
					document.kspayForm.action = "./payConfirm_EPay.jsp";
					document.kspayForm.submit();		
									
			}else{
				//LPAY 인증실패	
					install_notice_off();
				  document.getElementById("LPAYFRAME").style.visibility="hidden";
			}
		}
		...
		function paramSet(CARD_TRADE_AMT,TERMID,MGIFT_CARD_YN,CARD_DATE_NO,MGIFT_CONFIRM_NO,CARD_CERT_FLAG,CARD_ETC_DATA,DELEGATE_CERTIFY_CODE,MGIFT_CARD_NO,INSTALL_MONTH,OID,MGIFT_TRADE_AMT,CARD_CERTFY_NO,CARD_TRACK2_DATA,CARD_NO, SSGPAY_CARD_YN, SSGPAY_PAYMETHOD, SSGPAY_PLATFORM_MID)		{
			var frm = document.SSGPayAuthForm;
		...
		function recvResult_payco(proceed){
			...
			if(proceed == "TRUE"||proceed == "true"||proceed == true){	
				    var width = $('.innopay_wrap').width();
				    parent.$("#divpop").css('width',width);
					pay_frm.action = "./payConfirm_EPay.jsp";
					pay_frm.submit();	
				
			}else{		
				  document.getElementById("PAYCOFRAME").style.visibility="hidden";
					EPay_btn_off();
			}
		}
	...
	function EPay_btn_on(){   //페이코 전용 버튼 출력 함수.
	...
	function EPay_btn_off(){ //페이코 전용 버튼 숨김 && 프레임 width 원상 복구.
	  var btn=document.getElementById('bt');
							...
							<section class="payment_input">
								...
								<div class="card_list">
									<div class="simple_pay_list">
										<a href="#" class="on" id="simple_pay1_area" data-tooltip-text="카카오페이">
											<p class="simple_pay1" ></p>
										</a>
										<a href="#" id="simple_pay2_area" data-tooltip-text="엘페이">
											<p class="simple_pay2"></p>
										</a>
										<a href="#" id="simple_pay3_area" data-tooltip-text="페이코">
											<p class="simple_pay3"></p>
										</a>
										<a href="#" id="simple_pay4_area" data-tooltip-text="SSG페이(신용)">
											<p class="simple_pay4"></p>
										</a>
											<a href="#" id="simple_pay5_area" data-tooltip-text="SSG페이(머니)">
											<p class="simple_pay5"></p>
										</a>
										<a href="#" id="simple_pay6_area" data-tooltip-text="네이버페이">
...
<input type="hidden" name="TID"             	value="<%=TID%>"/>
	<input type="hidden" name="storeid"    		value="<%=Kakao_Key%>">         <!-- KSNET에서 따로 발급 받아야함. -->
<!--일반신용카드------------------------------------------------------>
	...
	<input type="hidden" name="boss_nm"        	value="<%=boss_nm%>"/>
...
<!-- 페이코 응답 파라미터 -->
<form name="paycoForm"  method="post" id="paycoForm" accept-charset="euc-kr">
	...
	<input type="hidden" name="sndBizNo"        	value="<%=co_no %>" >       <!-- 가맹점 사업자번호 -->	
	<input type="hidden" name="sndCharset"      	value="euc-kr" >                 <!-- 가맹점 charset -->	
</form>
...
<form name=SSGPayAuthForm method=post>
<!--기본-------------------------------------------------------------->
	<input type="hidden" name="sndStoreid"    	value="<%=Ssg_Key%>">           <!-- 상점아이디 -->
	<input type="hidden" name="sndEmail"		value="<%=BuyerEmail%>">           <!-- 이메일 -->
	<input type="hidden" name="sndMobile"		value="<%=BuyerTel%>">           <!-- 연락처 -->
	<input type="hidden" name="sndOrdernumber"	value="<%=Moid %>">           <!-- 주문번호 -->
	<input type="hidden" name="sndOrdername"	value="<%=BuyerName %>">           <!-- 주문자명 -->
...
<input type="hidden" name="ReturnURL"        			 value="<%=ReturnURL%>"/>
<input type="hidden" name="OrderCode"    				 value="<%=Moid%>">
...
<iframe name='LPAYFRAME' id='LPAYFRAME' src='' marginwidth='0' marginheight='0' frameBorder=0  scrolling=no ></iframe>	 
<iframe name='PAYCOFRAME' id='PAYCOFRAME' src='' marginwidth='0' marginheight='0' frameBorder=0  scrolling=no ></iframe>	
<iframe name='SFRAME' id='SFRAME' src='' marginwidth='0' marginheight='0' frameBorder=0  scrolling=no ></iframe>
<iframe name='SSGFRAME' id='SSGFRAME' src='' marginwidth='0' marginheight='0' frameBorder=0 ></iframe>
<iframe name='NAVERFRAME' id='NAVERFRAME' src='' marginwidth='0' marginheight='0' frameBorder=0 ></iframe>
	</body>
</html>