...
<%@ page
	import="util.CommonUtil, util.CardUtil, service.*, java.text.SimpleDateFormat"%>
...
<%
	...
	String sms_yn = CommonUtil.getDefaultStr(request.getParameter("sms_yn"), "N");
	String join_type = CommonUtil.getDefaultStr(request.getParameter("join_type"), "2");
	...
	req.put("pg_cd", pg_cd);
	if(!MMSUtil.isNullOrEmpty(mid)){
		if(cardEvent.isEtland(req)){
			req.put("pg_cd", "99");//전자랜드 99
			req.put("isDirect", "true");
		}else if("a00050994m".equals(mid)){//익수제약 98
			req.put("pg_cd", "98");
			req.put("isDirect", "true");
		}else if("seutopiacm".equals(mid)){//유토피아추모관 97
			req.put("pg_cd", "97");
			req.put("isDirect", "true");
		}else if("seutopiafm".equals(mid)){//유토피아퓨처 96
			req.put("pg_cd", "96");
			req.put("isDirect", "true");
		}
	}
 ...
 */
	...
	eventList = cardEvent.getCardEventMainWithMid(req);
	...
	<section class="wrap">
		...
		<div class="tab_wrap">
			...
			for (DataModel dm : eventList){
				%>
				<li><a href="#" <% if(pg_cd.equals(dm.getStrNull("pg_cd"))){%>class="on"<%} %>><% if(join_type.equals("2")){ out.print(dm.getStrNull("pg_nm")); } else { out.print("이노페이(PG)"); }%></a></li>
				<%
			}
			%>
			</ul>
		...
		<div class="data_wrap">
			...
			<%if(sms_yn.equals("N")){ %>
			<li>PG사 : <% if(join_type.equals("2")){ out.print(dm.getStrNull("pg_nm")); } else { out.print("이노페이(PG)"); }%></li>
			<%} %>
			</ul>
