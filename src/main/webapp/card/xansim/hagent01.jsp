...
<%@ page errorPage="error.jsp?status=agent01" %>
...
<%
    ...
    // [필수] 호스팅 쇼핑몰 응답받을 URL :
    String X_MERCHANT_RECEIVEURL="http://localhost:8080/ipay/card/xansim/hagent02.jsp";
    if (request.getServerPort() == 80 || request.getServerPort() == 443) {
        X_MERCHANT_RECEIVEURL = request.getScheme() + "://" + request.getServerName() +"/ipay/card/xansim/hagent02.jsp";
    } else {
        X_MERCHANT_RECEIVEURL = request.getScheme() + "://" + request.getServerName() + ":"+request.getServerPort()+"/ipay/card/xansim/hagent02.jsp";
    } 
...
//  -----------------------------------------------
    ...
    if ("SAMSUNGCARD".equals(X_CARDTYPE)){
        X_HOSTURL="https://vbv.samsungcard.co.kr/VbV/host/hostAgent01.jsp";
        X_RECEIVEURL="https://vbv.samsungcard.co.kr/VbV/host/hostAgent12.jsp";
    }else if ("HYUNDAICARD".equals(X_CARDTYPE)){
        X_HOSTURL="https://ansimclick.hyundaicard.com/host/hostAgent01.jsp";
        X_RECEIVEURL="https://ansimclick.hyundaicard.com/host/hostAgent12.jsp";
    }else if ("SHINHANCARD".equals(X_CARDTYPE)){
        X_HOSTURL="https://vbv.shinhancard.com/host/hostAgent01.jsp";
        X_RECEIVEURL="https://vbv.shinhancard.com/host/hostAgent12.jsp";
    }else if ("NONGHYUPCARD".equals(X_CARDTYPE)){
        X_HOSTURL="https://vbv.nonghyup.com/host/hostAgent01.jsp";
        X_RECEIVEURL="https://vbv.nonghyup.com/host/hostAgent12.jsp";
    }else {
...
function openXansim() {
	 var url = $('#sh').attr('src');
	interval = setInterval("unloadcheck()",1000);
	   if("<%=X_CARDTYPE%>"=="SHINHANCARD")
	   {
		  tar="sh";
	   }
	   else{ 
	   ...
	   document.X_ANSIMFORM.target=tar;
...
function unloadcheck(){
	  ...
	  if('<%=X_CARDTYPE%>'=='SHINHANCARD')
		  {
		  childwin =parent.$("#sh").css("visibility");
		  }
	  else
		  {
	 childwin =parent.$("#kbframe2").css("visibility");
		  }	  
    if (childwin=="hidden"){   	
    	if (!certResult){    		
...
<form name="X_ANSIMFORM" method="post">
    ...
    <input type=hidden name="X_HS_USEYN_SH" value="<%=X_HS_USEYN_SH%>">
    <!-- NH카드관련 추가 파라미터 -->
